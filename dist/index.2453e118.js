const t="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",e="nameZh1";//'YOUR_GITHUB_ACCESS_TOKEN';token
// 创建全局变量(当前所在路径)
...
l.append("file",a);let r=new FileReader;// 创建 FileReader 对象，用于读取文件内容
r.readAsDataURL(a),r.onload=function(){let i=r.result.split(",")[1];// 获取Base64编码后的文件内容
...
c(n)}).catch(t=>{console.error(t)})}}(n)}),// add('img/icon');
// del('img/1.jpg');
// search();
...
// 构建 API 请求的 URL
let l=`https://api.github.com/repos/${e}/img/contents/img`,r={Authorization:`token ${t}`},d=document.createDocumentFragment();// 发起 GET 请求获取仓库内容
fetch(l,{headers:r}).then(t=>t.json()).then(t=>{console.log(t,5555555),t.forEach(t=>{if("dir"==t.type){let e=document.createElement("div");e.textContent=t.name,e.setAttribute("path",t.path),e.className="imgContainer-content-left-foldeNode",// 点击文件夹时间
e.addEventListener("click",function(t){let e=t.target.getAttribute("path");n=e,o.textContent=e,c(e)}),d.appendChild(e)}}),a.appendChild(d)}).catch(t=>console.error("发生错误：",t))}(0);//# sourceMappingURL=index.2453e118.js.map

//# sourceMappingURL=index.2453e118.js.map
